...
internal sealed class RunRecord
{
    ...
    public static RunRecord Create(Profile p, string command, DateTimeOffset started, DateTimeOffset ended, int exitCode, string reason, ServerStats stats, string logPath = "", IEnumerable<RunResourceSample>? resourceSamples = null, string graphDataPath = "") => new()
    {
        ...
        Parallel = p.Parallel, Reasoning = p.Reasoning, ReasoningEffort = p.ReasoningEffort, ReasoningBudget = p.ReasoningBudget, Mtp = p.Mtp, MtpDraftTokens = p.MtpDraftTokens, NoMmap = p.NoMmap, ExtraArgs = [.. p.ExtraArgs],
...
internal static class RunHistory
{
    ...
    static bool SameScenario(RunRecord s, Profile p) => s.Model == p.Model && s.Ctx == p.Ctx && s.Ngl == p.Ngl && s.CacheK == p.CacheK && s.CacheV == p.CacheV && s.Batch == p.Batch && s.UBatch == p.UBatch && s.Parallel == p.Parallel && s.Reasoning == p.Reasoning && s.ReasoningEffort == p.ReasoningEffort && s.ReasoningBudget == p.ReasoningBudget && s.Mtp == p.Mtp && (!p.Mtp || s.MtpDraftTokens == p.MtpDraftTokens) && s.NoMmap == p.NoMmap && (s.ExtraArgs ?? []).SequenceEqual(p.ExtraArgs);
}
